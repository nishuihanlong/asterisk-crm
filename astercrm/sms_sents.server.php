...
require_once ('sms_sents.grid.inc.php');
...
function init(){
	...
	$objResponse->addAssign("btnSmsTemplates","value",$locate->Translate("sms_templates"));
...
function createGrid($start = 0, $limit = 1, $filter = null, $content = null, $order = null, $divName = "grid", $ordering = "",$stype=array()){
	...
	$headers[] = $locate->Translate("AgentName");
	$headers[] = $locate->Translate("Callerid");
	$headers[] = $locate->Translate("Target");
	...
	$eventHeader[]= 'onClick=\'xajax_showGrid(0,'.$limit.',"'.$filter.'","'.$content.'","target","'.$divName.'","ORDERING");return false;\'';
	...
	$fieldsFromSearch[] = 'callerid';
	$fieldsFromSearch[] = 'target';
	$fieldsFromSearch[] = 'content';
	$fieldsFromSearch[] = 'cretime';

	// Selecct Box: Labels showed on search select box.
	$fieldsFromSearchShowAs = array();
	$fieldsFromSearchShowAs[] = $locate->Translate("AgentName");
	$fieldsFromSearchShowAs[] = $locate->Translate("Callerid");
	$fieldsFromSearchShowAs[] = $locate->Translate("Target");
	...
	$table->addRowSearchMore("sms_sents",$fieldsFromSearch,$fieldsFromSearchShowAs,$filter,$content,$start,$limit,0,1,$typeFromSearch,$typeFromSearchShowAs,$stype);
	...
	// Change here by the name of fields of its database table
		...
		$rowc[] = $row['content'];
		$rowc[] = $row['cretime'];
		$table->addRow("sms_sents",$rowc,0,1,0,$divName,$fields);
 	}
...
function searchFormSubmit($searchFormValue,$numRows = null,$limit = null,$id = null,$type = null){
	...
	if($optionFlag == "export"  || $optionFlag == "exportcsv"){
		...
		$sql = astercrm::getSql($searchContent,$searchField,$searchType,'sms_sents'); //得到要导出的sql语句
		
		$_SESSION['export_sql'] = $sql;
		
		$objResponse->addAssign("hidSql", "value", $sql); //赋值隐含域
		$objResponse->addAssign("maintable", "value", 'sms_sents'); //赋值隐含域
