...
require_once ('note_leads.grid.inc.php');
...
function init(){
	...
	$objResponse->addAssign("btnCustomerLeads","value",$locate->Translate("customer_leads"));
...
function createGrid($start = 0, $limit = 1, $filter = null, $content = null, $order = null, $divName = "grid", $ordering = "",$stype=array()){
	...
	$eventHeader[]= 'onClick=\'xajax_showGrid(0,'.$limit.',"'.$filter.'","'.$content.'","note_leads.cretime","'.$divName.'","ORDERING");return false;\'';
	$eventHeader[]= 'onClick=\'xajax_showGrid(0,'.$limit.',"'.$filter.'","'.$content.'","note_leads.creby","'.$divName.'","ORDERING");return false;\'';
	...
	$fieldsFromSearch[] = 'customer_leads.customer';
	$fieldsFromSearch[] = 'note_leads.callerid';
	$fieldsFromSearch[] = 'note_leads.cretime';
	$fieldsFromSearch[] = 'note_leads.creby';
...
function searchFormSubmit($searchFormValue,$numRows = null,$limit = null,$id = null,$type = null){
	...
	if($optionFlag == "export" || $optionFlag == "exportcsv"){
		$sql =& Customer::getExportSql($searchContent,$searchField,$searchType,'note_leads'); //得到要导出的sql语句
		...
		astercrm::deletefromsearch($searchContent,$searchField,$searchType,'note_leads');
...
function deleteByButton($f,$searchFormValue){
	...
	if(is_array($f['ckb'])){
		foreach($f['ckb'] as $vaule){
			$res = astercrm::deleteRecord($vaule,'note_leads');
		}
	}
	$searchContent = $searchFormValue['searchContent'];  //搜索内容 数组
	$searchField = $searchFormValue['searchField'];      //搜索条件 数组
