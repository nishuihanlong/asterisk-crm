<?php
/*******************************************************************************
* speeddial.server.php

* Function Desc
	provide speeddial management script
...
require_once ('speeddial.grid.inc.php');
...
function createGrid($start = 0, $limit = 1, $filter = null, $content = null, $order = null, $divName = "grid", $ordering = "",$stype=array()){
	...
	$fields = array();
	$fields[] = 'speeddial.number';
	$fields[] = 'speeddial.description';
	$fields[] = 'astercrm_accountgroup.groupname';
	$fields[] = 'speeddial.creby';
	$fields[] = 'speeddial.cretime';

	// HTML table: Headers showed
	$headers = array();
	$headers[] = $locate->Translate("Number");
	$headers[] = $locate->Translate("Description");
	...
	$eventHeader = array();
	$eventHeader[]= 'onClick=\'xajax_showGrid(0,'.$limit.',"'.$filter.'","'.$content.'","speeddial.number","'.$divName.'","ORDERING");return false;\'';
	$eventHeader[]= 'onClick=\'xajax_showGrid(0,'.$limit.',"'.$filter.'","'.$content.'","speeddial.description","'.$divName.'","ORDERING");return false;\'';
	$eventHeader[]= 'onClick=\'xajax_showGrid(0,'.$limit.',"'.$filter.'","'.$content.'","astercrm_accountgroup.name","'.$divName.'","ORDERING");return false;\'';
	$eventHeader[]= 'onClick=\'xajax_showGrid(0,'.$limit.',"'.$filter.'","'.$content.'","speeddial.creby","'.$divName.'","ORDERING");return false;\'';
	$eventHeader[]= 'onClick=\'xajax_showGrid(0,'.$limit.',"'.$filter.'","'.$content.'","speeddial.cretime","'.$divName.'","ORDERING");return false;\'';

	// Select Box: fields table.
	$fieldsFromSearch = array();
	$fieldsFromSearch[] = 'speeddial.number';
	$fieldsFromSearch[] = 'speeddial.description';
	$fieldsFromSearch[] = 'astercrm_accountgroup.groupname';
	$fieldsFromSearch[] = 'speeddial.creby';
	$fieldsFromSearch[] = 'speeddial.cretime';
	...
	$table->addRowSearchMore("speeddial",$fieldsFromSearch,$fieldsFromSearchShowAs,$filter,$content,$start,$limit,1,0,$typeFromSearch,$typeFromSearchShowAs,$stype);
	while ($arreglo->fetchInto($row)) {
	// Change here by the name of fields of its database table
		$rowc = array();
		$rowc[] = $row['id'];
		$rowc[] = $row['number'];
		$rowc[] = $row['description'];
		$rowc[] = $row['groupname'];
		$rowc[] = $row['creby'];
		$rowc[] = $row['cretime'];
		$table->addRow("speeddial",$rowc,1,1,0,$divName,$fields);
 	}
 	// End Editable Zone
...
function searchFormSubmit($searchFormValue,$numRows = null,$limit = null,$id = null,$type = null){
	...
	$divName = "grid";
	if($exportFlag == "1"){
		$sql = astercrm::getSql($searchContent,$searchField,'speeddial'); //得到要导出的sql语句
		$_SESSION['export_sql'] = $sql;
		$objResponse->addAssign("hidSql", "value", $sql); //赋值隐含域
		$objResponse->addScript("document.getElementById('exportForm').submit();");
	}else{
		if($type == "delete"){
			$res = Customer::deleteRecord($id,'speeddial');
			if ($res){
...
function add(){
   // Edit zone
	global $locate;
	$objResponse = new xajaxResponse();
	$html = Table::Top($locate->Translate("Add SpeedDial number"),"formDiv");  // <-- Set the title for your form.
...
*/
	$respOk = Customer::insertNewSpeedDial($f); // add a new trunkinfo
	if ($respOk){
		$html = createGrid(0,ROWSXPAGE);
		$objResponse->addAssign("grid", "innerHTML", $html);
		$objResponse->addAssign("msgZone", "innerHTML", $locate->Translate("Add SpeedDial Number"));
...
function edit($id){
	global $locate;
	$html = Table::Top( $locate->Translate("Edit SpeedDial"),"formDiv"); 
